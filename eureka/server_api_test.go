...
func Test_DeRegisterInstance(t *testing.T) {
	...
	config.ServiceUrl = map[string]string{
		DEFAULT_ZONE: "http://hddata:hddata$2019@192.168.1.53:8002/eureka,http://hddata:hddata$2019@192.168.1.59:8002/eureka",
	}
	endpointUtils := new(EndpointUtils)
	urls, err := endpointUtils.GetDiscoveryServiceUrls(config, DEFAULT_ZONE)
	if err != nil {
		t.Fatal(err.Error())
	}
	...
	err = NewEurekaServerApi(urls[0]).DeRegisterInstance("gw-test-eureka",
		genDefaultInstanceId("gw-test-eureka", 80))
	if err != nil {
		t.Fatal(err.Error())
	}
